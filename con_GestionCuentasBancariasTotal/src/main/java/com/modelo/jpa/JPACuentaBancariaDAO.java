...
public class JPACuentaBancariaDAO extends JPAGenericDAO<CuentaBancaria, Integer> implements CuentaBancariaDAO {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<CuentaBancaria> listarCuentasPorPropietario(int idPropietario) {
		Query query = this.em.createNamedQuery("listarCuentasPorPropietario");
		query.setParameter("idPropietario", idPropietario);
		return (List<CuentaBancaria>) query.getResultList();		
	}
