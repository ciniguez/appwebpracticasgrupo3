...
import sin.modelo.dao.UsuarioDAO;
	...
	private void ingresar(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (resultado == null) {
			...
			if(resultado.isAdmin()) {
				sesionSitio.setAttribute("admin", "true");
				// le prmito pasar (Listar usuarios)
				resp.sendRedirect("GestionarUsuariosController?ruta=listar");
				
			} else {
				sesionSitio.setAttribute("admin", "false");
				// le prmito pasar (Listar usuarios)
				resp.sendRedirect("GestionarCuentasController?ruta=listar");
			}
