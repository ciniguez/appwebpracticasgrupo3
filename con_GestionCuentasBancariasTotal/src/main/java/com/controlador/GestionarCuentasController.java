...
public class GestionarCuentasController extends HttpServlet {
	...
	private void listarCuentas(HttpServletRequest request, HttpServletResponse response)
		...
		Usuario logeado = (Usuario) request.getSession().getAttribute("autorizado");
		List<CuentaBancaria> cuentas = null;
		//Si es administrador, muestro todas las cuentas.
		if(logeado.isAdmin()) {
			//Si no es administrador, muestro las cuentas del usuario logeado.
			cuentas = DAOFactory.getFactory().getCuentaBancariaDAO().listarCuentas();
		}else {
			
			//Si no es administrador, muestro las cuentas del usuario logeado.
			cuentas = DAOFactory.getFactory().getCuentaBancariaDAO().listarCuentasPorPropietario(logeado.getId());
